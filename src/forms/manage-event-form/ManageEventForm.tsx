    ...
    type: z.string({
        ...
        city: z.string({
            ...
            description: z.string({
                ...
                eventItems: z.array(z.object({
                    name: z.string().min(1, "Navn er påkrævet"),
                    price: z.coerce.number().min(0, "Pris skal være et positivt tal"),
                })
            ),
            imageUrls: z.array(z.string()).optional(),
            imageFile: z.instanceof( File, { message: 'Billede er påkrævet' }).optional(),
...
const ManageEventForm = ({ onSave, isLoading, event }: Props) => {
    ...
    const onSubmit = (formDataJson: EventFormData) => {
        ...
        formData.append('numberOfParticipants', formDataJson.numberOfParticipants.toString());
        formDataJson.facilities.forEach((facility, index) => {
            formData.append(`facilities[${index}]`, facility);
        });
        formDataJson.eventItems.forEach((eventItem, index) => {
            formData.append(`eventItems[${index}][name]`, eventItem.name);
            formData.append(`eventItems[${index}][price]`, eventItem.price.toString());
        });
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 bg-gray=50 p-10 rounded-lg">
                ...
                <Separator />
                <ImageSection />
                {isLoading ? <LoadingButton /> : <Button type="submit">Gem begivenhed</Button>}
            </form>
        </Form>
    );
};
